...
interface IImageMapProps {
    ...
    featureStyler?: (feature:Feature) => Style;
    tableBorderFeatureStyler?: (feature:Feature) => Style;
    tableIconFeatureStyler?: (feature:Feature, resolution) => Style;
    tableIconBorderFeatureStyler?: (feature:Feature) => Style;
    checkboxFeatureStyler?: (feature:Feature) => Style;
    podFeatureStyler?: (feature:Feature) => Style;
    labelFeatureStyler?: (feature:Feature) => Style;
    drawRegionStyler?: () => Style;
    drawnRegionStyler?: (feature:Feature) => Style;
    modifyStyler?: () => Style;
    ...
    handleFeatureSelect?: (feature: Feature, isTaggle: boolean, category: FeatureCategory) => void;
    handleFeatureDoubleClick?: (feature: Feature, isTaggle: boolean, category: FeatureCategory) => void;
    ...
    handleFeatureSelectByGroup?: (feature:Feature) => IRegion;
    ...
    updateFeatureAfterModify?: (features:Feature) => void;
...
export class ImageMap extends React.Component<IImageMapProps> {
    ...
    public toggleLabelFeatureVisibility = (visible: boolean = false) => {
        ...
        if (!drawnLabelLayerVisibility) {
            drawnLabelFeatures?.forEach((feature:Feature) => {
    ...
    public toggleDrawnRegionsFeatureVisibility = (visible: boolean = false) => {
        ...
        if (!drawnRegionLayerVisibility) {
            drawnRegionFeatures?.forEach((feature:Feature) => {
                this.removeFromDrawnFeatures(feature);
            });
        } else {
            drawnRegionFeatures?.forEach((feature:Feature) => {
    ...
    private pushToDrawnFeatures = (feature:Feature, drawnFeatures: Collection<Feature> = this.drawnFeatures) => {
    ...
    private removeFromDrawnFeatures = (feature:Feature, drawnFeatures: Collection<Feature> = this.drawnFeatures) => {
        const itemAlreadyExists = drawnFeatures.getArray().indexOf(feature) !== -1;
        if (itemAlreadyExists) {
    ...
    public removeAllDrawnLabelFeatures = () => {
        this.getAllDrawnLabelFeatures().forEach((feature:Feature) => {
            this.removeFromDrawnFeatures(feature);
        });
        this.drawnLabelLayer?.getSource().clear();
    };
